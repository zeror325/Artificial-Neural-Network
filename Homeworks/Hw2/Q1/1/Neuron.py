...
class Neuron():
    ...
    def start(self):
        if self.biased == False:
            self.weights = 0.01*np.sqrt(2/self.dimension)*np.random.rand(self.dimension+1)  #He baslatma kurali uygulanarak baslatildi 
            self.data = np.append(self.data,[1])
        if self.biased == True:
            self.weights = 0.01*np.sqrt(2/(self.dimension-1))*np.random.rand(self.dimension)  #He baslatma kurali uygulanarak baslatildi
    ...
    def func_act(self,a,lin_com):
        return 1/(1+np.exp(-(a*lin_com)))

    #AKTIVASYON FONKSIYONU TUREV
    def func_act_der(self,a,lin_com):
        return a*(np.exp((-a*lin_com))/((1+np.exp((-a*lin_com))**2)))
